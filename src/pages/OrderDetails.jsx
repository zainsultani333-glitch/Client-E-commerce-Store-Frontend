...
export default function OrderDetails() {
  ...
  useEffect(() => {
    api.get(`/orders/${id}`)
      ...
      .catch(err => { setError(err.response?.data?.message || "Order not found"); setLoading(false); });
  }, [id]);
    ...
    <>
      <style>{`
        @media print {
          ...
          #order-print-area { position: fixed; top: 0; left: 0; width: 100%; background: #fff !important; color: #000 !important; padding: 20px; }
      ...
      <div style={{ minHeight: "calc(100vh - 72px)", background: "var(--bg-base)", padding: "40px 24px" }}>
        <div className="container" style={{ maxWidth: "720px" }}>
          ...
          <div className="no-print" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
            ...
            <button onClick={handlePrint} className="btn-primary">🖨️ Print Order</button>
          </div>

          {/* ─── RECEIPT DOCUMENT ─── */}
          <div id="order-print-area" style={{ background: "var(--bg-card)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "20px", overflow: "hidden", boxShadow: "var(--shadow-lg)" }}>
            ...
            <div style={{ background: "linear-gradient(135deg, var(--bg-card), var(--bg-elevated))", borderBottom: "2px solid rgba(212,163,115,0.3)", padding: "36px 40px", textAlign: "center", position: "relative" }}>
              ...
              <p style={{ color: "var(--text-muted)", fontSize: "12px", letterSpacing: "2px", textTransform: "uppercase" }}>Official Order</p>
              <div style={{ marginTop: "16px", display: "inline-flex", alignItems: "center", gap: "6px", background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: "20px", padding: "5px 14px" }}>
                <span style={{ fontSize: "12px", color: "var(--success)", fontWeight: "700" }}>💵 {order.paymentMethod || "Cash on Delivery"}</span>
              </div>
            </div>
            ...
            <div style={{ padding: "32px 40px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", marginBottom: "28px" }}>
                <div>
                  ...
                  <div style={{ fontWeight: "700", fontSize: "16px", marginBottom: "4px" }}>{order.customerName}</div>
                  {order.phone && <div style={{ fontSize: "13px", color: "var(--text-secondary)", marginBottom: "2px" }}>📞 {order.phone}</div>}
                  {order.customerEmail && <div style={{ fontSize: "13px", color: "var(--text-muted)", marginBottom: "2px" }}>✉️ {order.customerEmail}</div>}
                  {order.address && <div style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "6px", lineHeight: 1.5 }}>📍 {order.address}</div>}
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: "11px", fontWeight: "700", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "8px" }}>Order Info</div>
                  <div style={{ fontFamily: "monospace", fontSize: "13px", color: "var(--primary)", fontWeight: "700", marginBottom: "4px" }}>#{String(order._id).slice(-8).toUpperCase()}</div>
              ...
              <div style={{ background: "linear-gradient(135deg, rgba(201,168,76,0.08), rgba(201,168,76,0.04))", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "12px", padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
                <div>
                  <div style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "4px" }}>
                    {order.products.length} item{order.products.length !== 1 ? "s" : ""} · {order.products.reduce((s, i) => s + i.quantity, 0)} pieces
                  </div>
                  <div style={{ fontWeight: "700", fontSize: "16px" }}>Grand Total</div>
                  <div style={{ fontSize: "12px", color: "var(--success)", marginTop: "2px" }}>💵 {order.paymentMethod || "Cash on Delivery"}</div>
                </div>
                <div style={{ fontSize: "30px", fontWeight: "900", color: "var(--primary)" }}>Rs. {order.totalAmount.toLocaleString()}</div>
              </div>
              ...
              <div style={{ textAlign: "center", padding: "16px 0 0", borderTop: "1px solid var(--border)" }}>
                ...
                <p style={{ fontSize: "11px", color: "var(--text-muted)" }}>Keep this order for your records. For queries: zainsultani333@gmail.com</p>
              </div>
            </div>
          </div>
